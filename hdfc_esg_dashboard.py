"""
╔═══════════════════════════════════════════════════════════════════════════════╗
║  HDFC Bank: Green Finance & ESG Investment Tracker  v4.1                     ║
...
with st.sidebar:
    ...
    if DATA_OK:
        ...
        st.markdown("---")
        st.caption(f"📊 v4.1  ·  {len(master):,} records")
...
if st.session_state.page == "overview":
    ...
    # ── UPDATED: Quarter Pie is now a Bar chart ──
    ...
    with g1:
        ...
        f6 = px.bar(qd, x="Quarter", y="Investment_Amount_USD", color="Quarter",
                    color_discrete_sequence=[GRN,GRN2,GOLD,BLUE], text_auto=".2s")
    ...
    with g2:
        ...
        f7.add_trace(go.Scatter(x=yy["Year"],y=yy["YoY"],name="YoY Growth %", mode="lines+markers",line=dict(color=GOLD,width=2.5),
                                marker=dict(size=7,color=GOLD,line=dict(color=D_BG,width=2))), secondary_y=True)
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    for col,(ic,vl,lb) in zip([c1,c2,c3,c4],[
        ...
        ("⚠️",f"{hp}%","High-Risk Projects"),("🏅",(tco[:16]+"…") if len(tco)>16 else tco,"Top ESG Company"),
    ]):
    ...
    # ── UPDATED: Simplified Treemap (Max 2 levels) ──
    ...
    f6 = px.treemap(
        tree,
        path=[px.Constant("🌏 ESG Portfolio"), "Risk_Category","Sector"],
        values="Investment_Amount_USD", color="Investment_Amount_USD",
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    # ── UPDATED: Funnel replaced with Horizontal Bar ──
    ...
    st.markdown('<div class="sec-title">🔥 Regional Investment Heatmap</div>', unsafe_allow_html=True)
    ...
    hmap_lay(fh1,"🔥 Sector × Region Heatmap",400)
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    for col,(ic,vl,lb) in zip([c1,c2,c3],[
    ...
    # ── UPDATED: Added 3-Year Moving Average ──
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    # ── UPDATED: Parallel Coordinates replaced with Quadrant Matrix ──
    ...
    med_esg = pc["ESG_Score"].median()
    med_roi = pc["ROI_Percentage"].median()
    
    f8 = px.scatter(
        pc, x="ESG_Score", y="ROI_Percentage", color="Risk_Category",
        size="Investment_Amount_USD", hover_data=["Sector", "Company_Name"],
        color_discrete_map=RISK_C, opacity=0.7, size_max=20
    )
    ...
    f8.update_layout(
        paper_bgcolor=D_BG, plot_bgcolor=D_BG,
        font=dict(family="Plus Jakarta Sans,sans-serif", color=TXT, size=11),
        title=dict(text="🎯 ESG vs ROI Quadrant Matrix (Bubble Size = Investment)", font=dict(family="Space Grotesk,sans-serif", color=TXT, size=13), x=0.02),
        margin=dict(l=12, r=12, t=48, b=12), height=480,
        xaxis=dict(title="ESG Score", gridcolor=GRID, linecolor=LINE),
        yaxis=dict(title="ROI Percentage", gridcolor=GRID, linecolor=LINE)
    )
...
# ════════════════════════════════════════════════════════════════════════════════
    ...
    c1,c2,c3,c4,c5 = st.columns(5)
    for col,(ic,vl,lb) in zip([c1,c2,c3,c4,c5],[
        ("💵",f"${si2:.2f}B","Investment"),("🌍",f"{sc2:.2f}M T","CO₂ Reduced"),
        ("📈",f"{sr2:.2f}%","Avg ROI"),("🌿",f"{se2:.1f}","Avg ESG Score"),
        ("🏗️",f"{sp2:,}","Projects"),
    ]):
        with col: st.markdown(kpi(ic,vl,lb), unsafe_allow_html=True)
    ...
    with s2:
        ...
        f6=go.Figure(go.Waterfall(
            x=wf["Year"].astype(str).tolist(),y=wf["delta"].tolist(), measure=["absolute"]+["relative"]*(len(wf)-1),
            connector=dict(line=dict(color=MUT,width=1.5)),
            increasing=dict(marker=dict(color=GRN)), decreasing=dict(marker=dict(color="#FF5252")),
